...
Представьте, что вы управляете волшебной системой агентного ИИ в облаке. Пользователи продолжают отправлять запросы, каждый из которых имеет определенное количество токенов, и ваши ИИ-агенты должны понять, как их обработать. Иногда они разделяют запросы, иногда отбрасывают их, если они слишком большие, а иногда направляют их прямо через основную линию. Мы хотим выяснить:

- Сколько запросов мы получаем всего?
- Сколько мы разделяем по сравнению с отправкой напрямую?
- Как часто мы отбрасываем запросы (и с какой стоимостью)?
- Как все это влияет на использование ресурсов и общую стоимость (в долларах)?

Другими словами, мы хотим смоделировать повседневную жизнь нашей системы агентного ИИ и посмотреть, что происходит, когда дела идут тяжело — или, по крайней мере, когда запросы продолжают поступать.


## Наша структура симуляции

Для нашей симуляции мы надели шляпы Python и настроили игривую среду. Мы использовали библиотеки, такие как numpy, pandas и, возможно, tqdm для тех восхитительных индикаторов прогресса, которые позволяют нам наблюдать за выполнением нашей симуляции в реальном времени. Наши основные шаги выглядели примерно так:
...
3. Моделирование стоимости: Мы отслеживаем, сколько токенов использует каждый запрос, затем рассчитываем стоимость LLM (или многоагентной системы) и AKS (Azure Kubernetes Service) на основе использования — в долларах.
...
- Распределение токенов: Мы предположили, что пользовательские запросы имеют различные размеры, с определенной средней длиной и стандартным отклонением. Система (т.е. наша группа ИИ-агентов) может добавлять некоторые дополнительные токены для каждого запроса, потому что, ну, системы должны работать.
- Порог разделения: Если запрос превышал наш порог, мы разделяли его на меньшие части. Мы установили этот порог несколько произвольно, основываясь на том, что считали "типичным большим запросом".
...
Вы можете найти ноутбук [здесь](https://github.com/eosfor/scripting-notes/blob/8d0af479302fd43f40b73627d16f72d8a12c7d79/notebooks/en/agentic-ai-infra-simulation/sim.ipynb)
...
- Мы видим, что около 80% запросов разделяются. Это означает, что наш порог установлен так, что значительная часть запросов достаточно велика, чтобы быть разделенной на меньшие части.
- Мы отбрасываем относительно небольшое количество запросов (в среднем 591), что может быть приемлемо, если мы оптимизируем затраты, а не 100% покрытие.
